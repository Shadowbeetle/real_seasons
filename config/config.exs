# This file is responsible for configuring your application
# and its dependencies with the aid of the Config module.
#
# This configuration file is loaded before any dependency and
# is restricted to this project.

# General application configuration
import Config

config :real_seasons, []

# Configure the endpoint
config :real_seasons, RealSeasonsWeb.Endpoint,
  url: [host: "localhost"],
  adapter: Bandit.PhoenixAdapter,
  render_errors: [
    formats: [html: RealSeasonsWeb.ErrorHTML, json: RealSeasonsWeb.ErrorJSON],
    layout: false
  ],
  pubsub_server: RealSeasons.PubSub,
  live_view: [signing_salt: "2Dk5OIyR"]

# Configure the mailer
#
# By default it uses the "Local" adapter which stores the emails
# locally. You can see the emails in your browser, at "/dev/mailbox".
#
# For production it's recommended to configure a different adapter
# at the `config/runtime.exs`.
config :real_seasons, RealSeasons.Mailer, adapter: Swoosh.Adapters.Local

# Configure esbuild (the version is required)
config :esbuild,
  version: "0.25.4",
  real_seasons: [
    args:
      ~w(js/app.js --bundle --target=es2022 --outdir=../priv/static/assets/js --external:/fonts/* --external:/images/* --alias:@=.),
    cd: Path.expand("../assets", __DIR__),
    env: %{"NODE_PATH" => [Path.expand("../deps", __DIR__), Mix.Project.build_path()]}
  ]

# Configure tailwind (the version is required)
config :tailwind,
  version: "4.1.12",
  real_seasons: [
    args: ~w(
      --input=assets/css/app.css
      --output=priv/static/assets/css/app.css
    ),
    cd: Path.expand("..", __DIR__)
  ]

# Configure Quantum scheduler
config :real_seasons, RealSeasons.Scheduler,
  jobs: [
    # Run data refresh on the 1st of each month at 3:00 AM
    {"0 3 1 * *", {RealSeasons.DataPipeline, :refresh, []}}
  ]

# Configure Gettext
config :gettext, :default_locale, "hu"
config :real_seasons, RealSeasonsWeb.Gettext, default_locale: "hu", locales: ~w(hu en)

# Configure timezone database for DateTime.shift_zone!/2
config :elixir, :time_zone_database, Tz.TimeZoneDatabase

# Configure Elixir's Logger
config :logger, :default_formatter,
  format: "$time $metadata[$level] $message\n",
  metadata: [:request_id]

# Use Jason for JSON parsing in Phoenix
config :phoenix, :json_library, Jason

# Import environment specific config. This must remain at the bottom
# of this file so it overrides the configuration defined above.
import_config "#{config_env()}.exs"
